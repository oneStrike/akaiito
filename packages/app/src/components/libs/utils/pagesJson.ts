...
import type { TransformPagesConf } from 'src/components/libs/types'
import * as path from 'node:path'
import * as uniPagesHandler from '@dcloudio/uni-cli-shared/dist/json/pages.js'
...
class TransformPages {
  ...
  constructor(pagesPath = './src', config?: TransformPagesConf) {
    ...
    this.uniPagesHandler = uniPagesHandler
  ...
  getPages(pages = this.pagesJson.pages, root?: string): Pages[] {
    ...
    return pages.map((item: IterateObject) => {
      let style: IterateObject = {}
      if (item.style.navigationBar) {
        style = {
          navigationStyle: item.style.navigationBar?.style,
          navigationBarTitleText: item.style.navigationBar?.titleText,
          navigationBarTextStyle: item.style.navigationBar?.titleColor,
          navigationBarBackgroundColor:
            item.style.navigationBar?.backgroundColor,
        }
      } else {
        style = item.style
      }
