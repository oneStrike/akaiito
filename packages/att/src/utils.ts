...
export function getName(path: string, typings = false) {
 ...
 * @returns 类型定义字符串
 */
function generateTyping(schema: IterateObject[] | string) {
  if (!schema) {
    return ''
  }
  let str = ''
 ...
 * @returns 文件名
 */
export function getFileName(path: string, config: IterateObject, isTyping = false) {
 ...
 * @returns 共享配置对象
 */
export function shared(api: IterateObject, config: IterateObject) {
